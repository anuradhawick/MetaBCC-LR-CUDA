...
def run_filter(reads_path, output, ids=None):
    ...
    if ids:
        ...
        for record, truth in tqdm(
            zip(SeqIO.parse(reads_path, fmt), open(ids)),
            total=records,
            desc="Filtering reads longer than 1000bp",
        ):
...
def run_kmers(reads_path, output, k_size, threads):
    ...
    cmd = f""""{os.path.dirname(__file__)}/bin/count-kmers" "{reads_path}" "{output}/profiles/{k_size}-mers" {k_size} {threads}"""
